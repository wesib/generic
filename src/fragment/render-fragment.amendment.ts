...
>(
  ...
  return ComponentMember<RenderFragmentDef.Method, TClass, RenderFragmentDef.Method, TAmended>(
    ({ key, get, amend }: AeComponentMemberTarget<RenderFragmentDef.Method, TClass>) => amend({
        componentDef: {
          define(defContext) {
            defContext.whenComponent((context: ComponentContext<InstanceType<TClass>>) => {
              context.whenReady(() => {
                ...
                context
                  .get(FragmentRenderCtl)
                  .renderFragmentBy(renderer, RenderFragment$def(context, key, def));
              });
            });
          },
        },
...
function RenderFragment$def(
  ...
  const {
    target = ({ contentRoot }) => drekCharger(drekAppender(contentRoot), RenderFragment$defaultRem(key)),
  } = spec;
